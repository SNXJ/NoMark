 ...
 * </ol>
 */
public class FixedVideoView extends TextureView
...
//        LogUtil.d(TAG, String.format("onMeasure, fixedWidth=%d,fixedHeight=%d", fixedWidth, fixedHeight));
    }
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        event.setClassName(FixedVideoView.class.getName());
    }

    @Override
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
        super.onInitializeAccessibilityNodeInfo(info);
        info.setClassName(FixedVideoView.class.getName());
    }
...
//            LogUtil.d(TAG, "transformVideo, getResizedHeight=" + getResizedHeight() + "," + "getResizedWidth=" + getResizedWidth());
            return;
        }
        sx = (float) getResizedWidth() / (float) videoWidth;
        sy = (float) getResizedHeight() / (float) videoHeight;
//        LogUtil.d(TAG, "transformVideo, sx=" + sx);
//        LogUtil.d(TAG, "transformVideo, sy=" + sy);
        ...
        if (dw > width && dh < height) {// 如果要铺满控件，且等比缩放，则
            maxScale = (float) width / (float) dw;// <1,缩小的
//            LogUtil.d(TAG, "transformVideo, maxScale1=" + maxScale);
        }
        // case 2:图片宽度<控件宽度，高度>控件高度
        if (dw < width && dh > height) {// 如果要完全显示图片，且等比缩放，则
            maxScale = (float) height / (float) dh;// <1,缩小的
//            LogUtil.d(TAG, "transformVideo, maxScale2=" + maxScale);
        }

        // case 3:图片宽度>控件宽度，高度>控件高度
        if (dw > width && dh > height) {// 如果要铺满控件，且等比缩放，则
            maxScale = Math.min(width * 1.0f / dw, height * 1.0f / dh);// <1,缩小的
//            LogUtil.d(TAG, "transformVideo, maxScale3=" + maxScale);
        }
        // case 4:图片宽度<控件宽度，高度<控件高度
        if (dw < width && dh < height) {// 如果要完全显示图片，且等比缩放，则
            maxScale = Math.min(width * 1.0f / dw, height * 1.0f / dh);// >1,放大的
//            LogUtil.d(TAG, "transformVideo, maxScale4=" + maxScale);
        }


//        maxScale = Math.max(sx, sy);
//        maxScale = sx;
        if (this.matrix == null) {
            matrix = new Matrix();
        } else {
        ...
        maxScale = (float) Math.round(maxScale * 100) / 100;

//        LogUtil.d(TAG, "transformVideo, maxScale5=" + maxScale);

        setTransform(matrix);

        postInvalidate();

        rw = videoWidth * maxScale;
        rh = videoHeight * maxScale;
//        Log.d(TAG, "transformVideo, videoWidth=" + videoWidth + "," + "videoHeight=" + videoHeight);
//        LogUtil.d(TAG, "transformVideo, videoWidth=" + videoWidth * maxScale + "," + "videoHeight=" + videoHeight * maxScale);

        if (null != videoRealWH) {
            videoRealWH.getWh(videoWidth * maxScale, videoHeight * maxScale > fixedHeight ? fixedHeight : videoHeight * maxScale);
        }
    }
...
//
    ...
    public interface VideoRealWH {
        void getWh(float w, float h);

    }

    VideoRealWH videoRealWH;

    public void setRealWH(VideoRealWH wh) {
        this.videoRealWH = wh;
    }
...
//    public CropData getReal(CropData cropData) {
////        if (sy > sx) {//长大于宽的
////            float y = cropData.getY() / maxScale;
////            float x = (cropData.getX() / maxScale + 0.5f * getVideoWidth() - 0.5f * (fixedWidth / maxScale));
////            float h = cropData.getH() / maxScale;
////            float w = cropData.getW() / maxScale;
////            cropData.setX((int) x);
////            cropData.setY((int) y);
////            cropData.setH((int) h);
////            cropData.setW((int) w);
...
////
////            Log.i("========", "====xxxx==" + x + "====yyy===" + y + "====");
////            Log.i("========", "====wwww==" + w + "====hhhhh===" + h + "====");
//////            Log.i("========", "====wwww22==" + w2 + "====hhhhh22===" + h2 + "====");
////
////            return cropData;
////
////        } else {
//        int x = Math.round(cropData.getX() / maxScale);
//        int y = Math.round(cropData.getY() / maxScale);
//        int h = Math.round(cropData.getH() / maxScale);
//        int w = Math.round(cropData.getW() / maxScale);
//
//        cropData.setX(x);
//        cropData.setY(y);
//        cropData.setH(h);
